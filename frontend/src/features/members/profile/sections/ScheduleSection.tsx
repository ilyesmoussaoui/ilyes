...
export function ScheduleSection({ profile }: ScheduleSectionProps) {
  // Build a map: dayOfWeek -> list of entries
  ...
  profile.disciplines.forEach((discipline) => {
    const colorIdx = disciplineNames.indexOf(discipline.disciplineName) % DISCIPLINE_COLORS.length;
      ...
      {!hasSchedule ? (
        <Card>
          <div className="flex flex-col items-center gap-2 py-6 text-center">
            <p className="text-sm font-medium text-neutral-700">No scheduled sessions</p>
            <p className="text-xs text-neutral-500">
              This member has no recurring schedule. Add disciplines with schedules to see them here.
            </p>
          </div>
        </Card>
      ) : (
              ...
              return (
                <div
                  ...
                  className={`rounded-lg border p-2 ${
                    isActive ? 'border-primary-200 bg-primary-50/30' : 'border-neutral-100 bg-neutral-50'
                  }`}
                >
                  <p className={`mb-2 text-xs font-semibold ${isActive ? 'text-primary-700' : 'text-neutral-400'}`}>
                    {DAY_SHORT[dayIdx]}
                  </p>
                  {entries.length === 0 ? (
                    ...
                    <ul className="flex flex-col gap-1.5">
                      {entries.map((entry, i) => (
                        ...
                        >
                          <p className="font-medium truncate">{entry.disciplineName}</p>
                          <p className="opacity-80">{entry.startTime}–{entry.endTime}</p>
                        </li>
                      ))}
                    </ul>
                  )}
          ...
          <ul className="sm:hidden flex flex-col gap-3">
            {activeDays.map((dayIdx) => {
              ...
              return (
                <li key={dayIdx}>
                  <Card padding="sm">
                    <p className="mb-2 text-sm font-semibold text-neutral-900">{DAY_NAMES[dayIdx]}</p>
                    <ul className="flex flex-col gap-1.5">
                      {entries.map((entry, i) => (
                        <li
                          key={i}
                        ...
                        >
                          <span className="font-medium">{entry.disciplineName}</span>
                          <span className="text-xs opacity-80">{entry.startTime}–{entry.endTime}</span>
                        </li>
                      ))}
                    </ul>
                  </Card>
